        ...
        <SelectionGrid columns={2}>
          {waterSources.map((source) => (
            <SelectionCard
              ...
              onClick={() => setValue('waterSource', source.id, { shouldValidate: true })}
            />
          ))}
        </SelectionGrid>
        {errors.waterSource && (
          <p className="text-sm text-brick-red">{String(errors.waterSource.message)}</p>
        )}
      </div>
        ...
        <SelectionGrid columns={3}>
          {hotWaterSystems.map((system) => (
            <SelectionCard
              ...
              onClick={() => setValue('hotWaterSystem', system.id, { shouldValidate: true })}
            />
          ))}
        </SelectionGrid>
        {errors.hotWaterSystem && (
          <p className="text-sm text-brick-red">{String(errors.hotWaterSystem.message)}</p>
        )}
      </div>
