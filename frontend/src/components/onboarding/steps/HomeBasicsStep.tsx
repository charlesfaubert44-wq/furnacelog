  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        />
        {errors.homeName && (
          <p className="text-sm text-brick-red">{String(errors.homeName.message)}</p>
        )}
        ...
        <SelectionGrid columns={2}>
          {homeTypes.map((type) => (
            <SelectionCard
              ...
              onClick={() => setValue('homeType', type.id, { shouldValidate: true })}
            />
          ))}
        </SelectionGrid>
        {errors.homeType && (
          <p className="text-sm text-brick-red">{String(errors.homeType.message)}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          />
          {errors.community && (
            <p className="text-sm text-brick-red">{String(errors.community.message)}</p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {errors.territory && (
            <p className="text-sm text-brick-red">{String(errors.territory.message)}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="space-y-2">
          ...
          />
          {errors.yearBuilt && (
            <p className="text-sm text-brick-red">{String(errors.yearBuilt.message)}</p>
          )}
        </div>
